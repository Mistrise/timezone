...
export const CardActive = ({ timeFormat, city }: Props) => {
  const [isTapped, setIsTapped] = useState(false); // 🔵 состояние сжатия

  const gmtOffsetSeconds = useMemo(() => {
    return getTzOffsetByName(city.timezone) * 60;
  }, []);
  const gradientRef = useRef<HTMLDivElement>(null);
  const bgPositionPercentRef = useRef<number>(
    getBgPositionPercent(gmtOffsetSeconds, 0)
  );
  ...
  const timeZoneDate = useMemo(() => {
    const secondsToAdd =
      new Date().getTimezoneOffset() * 60 + gmtOffsetSeconds;
  ...
  useEffect(() => {
    const updateBgPosition = (secondsOffset: number) => {
      if (gradientRef.current) {
        const percent = getBgPositionPercent(
          gmtOffsetSeconds - -16600,
          secondsOffset
        );
  ...
  return (
    <div
      className={clsx(styles.card, { [styles.card_shrinked]: isTapped })}
      ref={gradientRef}
      onClick={() => setIsTapped((prev) => !prev)} // 🔵 переключаем состояние
      style={{
    ...
    >
      <div className={styles.card_grid}>
          ...
          <div className={`${styles.card__time__item}`}>
            ...
            {hoursOffset === 0
              ? minutes < 10
                ? `0${minutes}`
                : minutes
              : Math.ceil(minutes / 30) * 30 !== 60
              ? "00"
              : "30"}
          </div>
          <span
            className={clsx(styles.card__am, {
        ...
        <div className={styles.card__timezone}>
          GMT {gmtOffset >= 0 ? `+${gmtOffset}` : gmtOffset}
        </div>
        <div
        ...
        >
          ...
          {`${days[timeZoneDate.getDay()]} ${timeZoneDate.getDate()} ${
            month[timeZoneDate.getMonth()]
          }`}
        </div>
      </div>
      <div className={styles.card_controls}>
        ...
        <div className={styles.card_control}>
          <Image src={Drag} width={24} height={24} alt="" />
        </div>
      </div>
    </div>
  );
