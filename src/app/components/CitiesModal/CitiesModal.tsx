...
const CitiesModal = ({setShowSearch, setCity, cities}:Props) => {
    ...
    const [filteredCity, set_City] = useState(citiesConst)

    function filterModalInput(input: string) {
        input === '' ?
            set_City(citiesConst) :
            set_City(filteredCity.filter(city => city.city.toLowerCase().startsWith(input.toLowerCase())))
    }
    ...
    return (
        <div className={styles.modal__container}>
            ...
            <ModalList modalInput={modalInput} cities={filteredCity} setCity={setCity} mainPageCities={cities}/>
        </div>
    )
}
